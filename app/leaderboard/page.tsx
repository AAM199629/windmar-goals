import { getAllMetrics } from '@/lib/kv'
import { CRUISE_TARGET, CRUISE_PERSONAL_TARGET, CRUISE_END } from '@/lib/config'
import type { GoalsMetrics } from '@/lib/metrics'
...
export default async function LeaderboardPage() {
  ...
  const rows = all
    .filter((m): m is GoalsMetrics & { cruise: NonNullable<GoalsMetrics['cruise']> } =>
      !!(m.cruise?.breakdown)
    )
    .sort((a, b) => (b.cruise.total ?? 0) - (a.cruise.total ?? 0))

  const withPoints  = rows.filter(m => (m.cruise.total ?? 0) > 0).length
  const qualified   = rows.filter(m => (m.cruise.total ?? 0) >= CRUISE_TARGET).length
  const endDate     = new Date(CRUISE_END + 'T00:00:00')
  const today       = new Date()
  const daysLeft    = Math.max(0, Math.ceil((endDate.getTime() - today.getTime()) / 86_400_000))

  const lastSync = rows.length > 0
    ? rows.reduce((l, m) => (m.updatedAt > l ? m.updatedAt : l), rows[0].updatedAt)
    : null
    ...
    <main style={{ minHeight: '100vh', background: 'var(--light)', paddingBottom: '3rem' }}>
      ...
      <section style={{
        ...
        <div style={{
          ...
          {/* ── Rule pills (static competition rules) ── */}
          <div style={{ display: 'flex', gap: '1px', flexWrap: 'wrap', marginBottom: '1.5rem' }}>
            {[
              ...
              <div key={big + label} style={{
                ...
                <div style={{
                  ...
                  color: 'rgba(255,255,255,0.55)', textTransform: 'uppercase', lineHeight: 1.3,
                }}>{label}<br />{sub}</div>
              </div>
            ))}
            ...
            {/* Live stats */}
            {[
              { big: String(rows.length),  label: 'VENDEDORES',  sub: 'EN COMPETENCIA',  accent: 'rgba(255,255,255,0.9)' },
              { big: String(daysLeft),      label: 'DÍAS',        sub: 'RESTANTES',        accent: '#7EC8E3' },
              { big: String(qualified),     label: qualified === 1 ? 'CLASIFICADO' : 'CLASIFICADOS', sub: 'HASTA AHORA', accent: qualified > 0 ? 'var(--gold)' : 'rgba(255,255,255,0.4)' },
              ...
              <div key={big + label} style={{
                ...
                <div style={{
                  fontFamily: 'var(--font-cond)', fontWeight: 700,
                  fontSize: '0.58rem', letterSpacing: '0.15em',
                  color: 'rgba(255,255,255,0.45)', textTransform: 'uppercase', lineHeight: 1.3,
                }}>{label}<br />{sub}</div>
              </div>
            ))}
          </div>
      ...
      <div className="section-eyebrow" style={{ background: 'var(--light)' }}>
        🏆 Clasificación General — {rows.length} vendedores · {withPoints} con puntos acumulados
      </div>
      ...
      <div style={{ padding: '0 1.5rem' }}>
        <div style={{
          ...
          boxShadow: '0 4px 24px rgba(13,22,84,0.12)',
          ...
          <table style={{
            ...
            <thead>
              ...
              <tr>
                ...
                <th colSpan={4} style={thGroup('Ventas por Producto', '#1565C0')}>
                  Ventas por Producto
                </th>
                <th colSpan={1} style={thGroup('Reclutamiento', '#E88B0C')}>
                  Reclutamiento
                </th>
                <th colSpan={1} style={thGroup('Asistidas', '#2196F3')}>
                  Asistidas
                </th>
              ...
              <tr style={{ background: 'var(--navy)' }}>
                {[
                  ...
                  <th key={label} style={{
                    ...
                    textAlign: align, width: w, whiteSpace: 'nowrap',
                  }}>{label}</th>
                ))}
              </tr>
            </thead>
            ...
            <tbody>
              {rows.map((m, i) => {
                const bd    = m.cruise.breakdown
                const total = m.cruise.total ?? 0
                const recl  = (bd.consultor ?? 0) + (bd.lider ?? 0) + (bd.gerente ?? 0)
                const pct   = Math.min(100, (total / CRUISE_TARGET) * 100)
                const { icon, color } = rankBadge(i + 1)
                const isQualified = total >= CRUISE_TARGET
                ...
                return (
                  <tr key={m.zohoId} style={{
                    ...
                    <td style={{
                      ...numTd,
                      color: recl > 0 ? 'var(--orange)' : '#ccd3e0',
                      fontWeight: recl > 0 ? 700 : 400,
                    }}>{recl > 0 ? fmt(recl) : '—'}</td>

                    {/* Asistidas */}
                    <td style={{
                      ...numTd,
                      color: (bd.asistida ?? 0) > 0 ? 'var(--blue)' : '#ccd3e0',
                    }}>{(bd.asistida ?? 0) > 0 ? fmt(bd.asistida ?? 0) : '—'}</td>
                    ...
                    <td style={{ padding: '0.7rem 1rem 0.7rem 0.5rem', textAlign: 'center', verticalAlign: 'middle' }}>
                      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '0.3rem' }}>
                        <span style={{
                          fontFamily: 'var(--font-bebas)',
                          fontSize: total > 0 ? '1.35rem' : '1rem',
                          letterSpacing: '0.04em', lineHeight: 1,
                          color: isQualified ? 'var(--orange)' : total > 0 ? 'var(--navy)' : '#ccd3e0',
                        }}>
                          {total > 0 ? fmt(total) : '0'}
                          <span style={{
                            fontFamily: 'var(--font-cond)', fontSize: '0.6rem',
                            color: '#aab4cc', marginLeft: 3, letterSpacing: '0.05em',
                          }}>/ {CRUISE_TARGET}</span>
                        </span>
                        {total > 0 && (
                          <div style={{ width: 72, height: 4, background: '#e4e9f5', borderRadius: 999, overflow: 'hidden' }}>
                            <div style={{
                              ...
                              background: isQualified
                                ? 'linear-gradient(to right, var(--orange), var(--gold))'
                                : 'linear-gradient(to right, var(--blue), #42A5F5)',
                              borderRadius: 999,
                            }} />
                          </div>
                        )}
              ...
              {rows.length === 0 && (
                <tr>
                  <td colSpan={9} style={{ textAlign: 'center', padding: '3rem', color: 'var(--gray)', fontFamily: 'var(--font-cond)' }}>
                    No hay datos disponibles. Pide al administrador que corra el sync.
                  </td>
                </tr>
              )}
            </tbody>
      ...
      {lastSync && (
        <footer className="dashboard-footer">
          Actualizado: {new Date(lastSync).toLocaleString('es-PR', { timeZone: 'America/Puerto_Rico' })}
        </footer>
      )}
    </main>
  )
